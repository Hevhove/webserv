...
void Server::run(void) {
    ...
    while (1) {
        ...
        for (int i = 0; i < _fd_count; i++) {
            // std::cout << "Events on pfds[" << i << "].revents: " <<
            // _pfds[i].revents << std::endl; If a client sends a POLLHUP
            // signal, it is asking to close the connection
            ...
            // Or if a descriptor is ready for writing, and a parsed request is
            // ready!
            else if (_pfds[i].revents & POLLOUT) {
                /*
                    Because of keep-alive connection, this event will trigger a
                   lot... as long as client keeps the connection open!
                */
...
void Server::respondToExistingConnection(int i) {
    ...
    // std::cout << "bytes sent on " << _pfds[i].fd << " is " << bytes_sent <<
    // std::endl;
...
void Server::readFromExistingConnection(int i) {
    ...
    if (nbytes <= 0) {
        if (nbytes == 0)
            std::cout << "---SOCKET "
                      << _connections[_pfds[i].fd]->getSocketFD()
                      << " HUNG UP---" << std::endl;
        else if (nbytes < 0)
...
void Server::handleNewConnection(void) {
    ...
    try {
        new_connection->setSocketFD(accept(_listenSocket.getSocketFD(),
...
void Server::dropConnection(int i) {
    ...
    //
    // Remove from the pfds list by copying the file descriptor of the last one
    // over it.
